...
> = {
  linkedin: { label: "LinkedIn", limit: 3000, note: "Post limit 3,000 characters", brand: "oklch(0.62 0.13 245)" },
  twitter: { label: "X / Twitter", limit: 280, note: "Tweet limit 280 characters", brand: "oklch(0.85 0.01 250)" },
  facebook: { label: "Facebook", limit: 2000, note: "Readable limit ~2,000 characters", brand: "oklch(0.6 0.17 265)" },
  instagram: { label: "Instagram", limit: 2200, note: "Caption limit 2,200 characters", brand: "oklch(0.68 0.19 10)" },
};
...
export function buildShareText(
  ...
  const tags = ["GitHubRewind", "BuildInPublic", ...langs.map((l) => l.replace(/[^A-Za-z0-9]/g, ""))]
    .slice(0, 5)
    .map((t2) => `#${t2}`)
    .join(" ");
  ...
  const body = [
    ...
    insights?.significantAchievements?.length
      ? `🏆 Highlights\n${insights.significantAchievements.slice(0, 3).map((a) => `• ${a}`).join("\n")}`
      : "",
    "",
